...
class AutoGLMApplication : Application() {
    ...
    override fun onCreate() {
        ...
        // Start AiMacrodroidService if Base URL is configured
        val settingsManager = SettingsManager.getInstance(this)
        if (settingsManager.getAiMacrodroidBaseUrl().isNotBlank()) {
            try {
                AiMacrodroidService.start(this)
            } catch (e: Exception) {
                Logger.e(TAG, "Failed to start AiMacrodroidService", e)
            }
        }
